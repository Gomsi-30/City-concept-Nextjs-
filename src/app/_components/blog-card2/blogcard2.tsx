...
const BlogCard2 = ({ data, section, extra, showContent = true }: gridData & { showContent?: boolean }) => {
  return (
    <div
      className={`grid gap-8 grid-cols-1 sm:grid-cols-1 ${
        data.length < 4 ? 'md:grid-cols-1 lg:grid-cols-1' : 'md:grid-cols-2 lg:grid-cols-2'
      }`}
    >
        ...
        >
          <div className="flex flex-col h-full cursor-pointer overflow-hidden">
            ...
            <div className="flex flex-col sm:gap-2 md:gap-0">
              <p className="text-md sm:text-md md:text-md lg:text-lg font-semibold text-gray-700">
                <CreationDate articleNumber={articleNumber} />
              </p>
              <p className="line-clamp-3 text-xl sm:text-xl md:text-md lg:text-md font-bold md:font-semibold text-gray-900">
                {title}
              </p>
              {showContent && (
